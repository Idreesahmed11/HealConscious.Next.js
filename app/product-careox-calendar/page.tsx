...
const GooglePayModal = ({ onClose }: { onClose: () => void }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div ref={modalRef} className="bg-white w-full max-w-sm rounded-lg shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "90vh" }}>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
          <h2 className="text-xl font-normal text-gray-900 mb-5">Add credit or debit card</h2>
          <div className="border border-gray-300 rounded px-3 py-3 mb-4">
            <input type="text" placeholder="Card number" value={cardNumber} onChange={(e) => setCardNumber(formatCard(e.target.value))} className="w-full text-sm outline-none text-gray-800 placeholder-gray-400" inputMode="numeric" />
          </div>
          <div className="flex gap-3 mb-4">
            <input type="text" placeholder="MM/YY" value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))} className="flex-1 border border-gray-300 rounded px-3 py-3 text-sm outline-none" />
            <input type="text" placeholder="CVV" value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, "").slice(0, 4))} className="flex-1 border border-gray-300 rounded px-3 py-3 text-sm outline-none" />
          </div>
          <input type="text" placeholder="Cardholder name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none mb-6" />
      ...
      <div ref={modalRef} className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "90vh" }}>
        ...
        <div className="px-8 py-8 flex flex-col items-center">
          ...
          <div className="w-full border border-gray-200 rounded-xl p-5 mb-5">
            ...
            <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-[#00D64F] rounded-lg px-4 py-3 text-sm outline-none" />
          </div>
          <button
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-14">
          <div className="w-full lg:w-[46%]">
            <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-md bg-stone-50" style={{ aspectRatio: "4/3" }}>
              <img src="/assets/Careox-Integrated-Printable-Page-Heal-Conscious-1536x1113.png" alt="Careox Printable Calendar" className="w-full h-full object-cover" />
            </div>
          </div>
          <div className="w-full lg:flex-1 flex flex-col">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <span className="text-xs text-gray-400 ml-1">(8 reviews)</span>
            </div>
            <h1 className="text-2xl sm:text-[1.75rem] font-semibold text-gray-900 leading-snug mb-3">Careox Program Printable Calendar</h1>
            <div className="flex items-baseline gap-3 mb-4">
              <span className="text-3xl font-bold text-gray-900">$15.00</span>
            </div>
            <p className="text-sm text-gray-500 leading-relaxed mb-4">A beautiful printable calendar to track your Careox program journey day by day.</p>
            <p className="text-sm text-gray-500 italic mb-1">Digital download — instant access after purchase</p>
            <p className="text-sm text-gray-500 mb-1">
              Read our <button onClick={() => setShowShipping(true)} className="underline text-gray-700 hover:text-gray-900">shipping policy here</button> before you make the purchase.
            </p>
            <p className="text-sm text-gray-500 mb-4">
              Read our <button onClick={() => setShowRefund(true)} className="underline text-gray-700 hover:text-gray-900">refund and exchange policy here</button> before you make the purchase.
            </p>
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-9 h-11 text-gray-500 hover:bg-gray-100 text-lg font-light">−</button>
                <span className="w-10 text-center text-sm font-medium text-gray-800">{qty}</span>
                <button onClick={() => setQty((q) => q + 1)} className="w-9 h-11 text-gray-500 hover:bg-gray-100 text-lg font-light">+</button>
              </div>
            ...
            <button className="w-full h-11 rounded-lg border-2 border-gray-900 text-sm font-semibold text-gray-900 hover:bg-gray-50 mb-4">BUY IT NOW</button>
            <div className="flex gap-3 mb-5">
              <button onClick={() => setShowGPay(true)} className="flex-1 h-11 rounded-xl bg-black flex items-center justify-center gap-1.5 hover:opacity-90 transition-all">
              ...
              <button onClick={() => setShowLink(true)} className="flex-1 h-11 rounded-xl bg-[#00D64F] flex items-center justify-center gap-1.5 hover:opacity-90 text-white font-semibold text-sm">
                <BoltIcon /> Pay with link
              </button>
            </div>
            <button onClick={() => setWishlist(!wishlist)} className="flex items-center gap-2 text-sm mb-6" style={{ color: wishlist ? "#ef4444" : "#9ca3af" }}>
          ...
          <div className="flex border-b border-gray-200">
            {["description", "reviews"].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-6 py-3.5 text-xs font-bold uppercase tracking-widest border-b-2 -mb-px ${activeTab === tab ? "border-gray-900 text-gray-900" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
                {tab === "reviews" ? "Reviews (8)" : "Description"}
              </button>
            ))}
          </div>
          <div className="py-10 text-gray-600 leading-relaxed text-[15px]">
            {activeTab === "description" ? (
              <div className="space-y-5 max-w-3xl">
                <p>The Careox Program Calendar is your go-to tool for building structure and balance in daily life.</p>
                <ul className="space-y-3">
                  {["Printable design for home or office use", "Sections for habits, notes and reminders", "Minimalist layout for clarity", "Perfect for program tracking"].map((item) => (
                    <li key={item} className="flex gap-3"><span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-gray-400 flex-shrink-0" /><span>{item}</span></li>
                  ))}
                </ul>
              </div>
            ) : (
                ...
                <div className="border border-gray-200 rounded-xl p-6">
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <input type="text" placeholder="Your name *" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none" />
                      <input type="email" placeholder="Your email *" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none" />
                    </div>
                    <textarea placeholder="Your review *" className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm h-32 outline-none resize-none" />
                    <button className="bg-gray-900 text-white px-8 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-gray-700">Submit Review</button>
                  </div>
                </div>
              </div>
            )}
      ...
      <section className="w-full border-t border-gray-100 mt-2 py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            <Link href="/product-fortification-program" className="group cursor-pointer">
              <div className="rounded-xl overflow-hidden bg-gray-50 border border-gray-100 mb-3" style={{ aspectRatio: "4/3" }}>
                <img src="/assets/careox-mental-health-guide-program.png" alt="4 Week Program" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              </div>
            ...
            <Link href="/product-leather-journal" className="group cursor-pointer">
              <div className="rounded-xl overflow-hidden bg-gray-50 border border-gray-100 mb-3" style={{ aspectRatio: "4/3" }}>
                <img src="/assets/careox-journal-front.png" alt="Leather Journal" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              </div>
