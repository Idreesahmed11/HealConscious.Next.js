      ...
      <div ref={modalRef} className="bg-white w-full max-w-sm rounded-lg shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "90vh" }}>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
          ...
          <div className="flex gap-3 mb-4">
            <input type="text" placeholder="MM/YY" value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))} className="flex-1 border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400" inputMode="numeric" />
            <input type="text" placeholder="CVV" value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, "").slice(0, 4))} className="flex-1 border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400" inputMode="numeric" />
          </div>
          <input type="text" placeholder="Cardholder name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400 mb-4" />
          <input type="text" placeholder="Street address" value={street} onChange={(e) => setStreet(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400 mb-4" />
          <input type="text" placeholder="City" value={city} onChange={(e) => setCity(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400 mb-4" />
          <input type="text" placeholder="Postal code" value={postal} onChange={(e) => setPostal(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none text-gray-800 placeholder-gray-400 mb-6" inputMode="numeric" />
...
const LinkModal = ({ onClose, price }: { onClose: () => void; price: string }) => {
...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {showLink && <LinkModal onClose={() => setShowLink(false)} price="199.00" />}
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-14">
          <div className="w-full lg:w-[46%]">
            <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-md bg-stone-50" style={{ aspectRatio: "4/3" }}>
              <img src="/assets/careox-mental-health-guide-program.png" alt="Careox 4 Week Program" className="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <div className="w-full lg:flex-1 flex flex-col">
            ...
            <p className="text-sm text-gray-500 leading-relaxed mb-3">
              Our self-help mental health guide is carefully curated and crafted by revered psychologist in their niche, and personal coaching and healing.
            </p>
            ...
            <button onClick={() => router.push("/careox-experience")} className="w-full h-11 rounded-lg border-2 border-gray-900 text-sm font-semibold text-gray-900 hover:bg-gray-50 transition-colors mb-4">
              BUY IT NOW
            </button>
          ...
          <div className="flex border-b border-gray-200">
            {["description", "reviews"].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab === "reviews" ? "Reviews (12)" : "Description"}
              </button>
            ))}
          </div>
          <div className="py-10 text-gray-600 leading-relaxed text-[15px]">
            {activeTab === "description" ? (
              <div className="space-y-5 max-w-3xl">
                <p>The Careox: Mental Healthcare in a Box is a guided self-help program designed by experts to help you rewire thought patterns, release emotional blocks, and rediscover your best self.</p>
                <p>Careox is a 28-day mental healthcare guide designed to help you release buried pain, ease daily stress, and break free from social pressures.</p>
                <div>
                  <h4 className="font-bold mb-4 text-gray-900 text-base">Key Features:</h4>
                  <ul className="space-y-3">
                    {["28-Day Guided Program", "Expertly Curated", "Holistic Healing Approach", "Perfect Gift for Loved Ones", "Portable & Easy to Use"].map((item) => (
              ...
              <div className="max-w-2xl">
                <p className="text-gray-600 mb-8">There are no reviews yet.</p>
                <div className="border border-gray-200 rounded-xl p-6 md:p-8">
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <input type="text" placeholder="Your name *" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-gray-400" />
                      <input type="email" placeholder="Your email *" className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-gray-400" />
                    </div>
                    <textarea placeholder="Your review *" className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm h-32 outline-none focus:border-gray-400 resize-none" />
